...
export default function CartView() {
  ...
  useEffect(() => {
    async function fetchCartProducts() {
      try {
        const response = await axios.get(`http://localhost:8080/api/user/cart/get-cart-products/${userID}`);
        const { data } = response;
  ...
  const handleRemoveProduct = async (productID) => {
    try {
      await axios.delete(`http://localhost:8080/api/user/cart/remove-product/${userID}/${productID}`);
